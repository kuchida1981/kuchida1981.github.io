# author-taxonomy Specification

## Purpose
Hugo の taxonomy として `author` を定義し、著者別の記事一覧ページを自動生成する。記事ページの author 名を対応する taxonomy ページへのリンクとして表示する。

## Requirements

### Requirement: Author taxonomy が有効であること
Hugo の taxonomy として `author` が定義されており、`/author/` および `/author/<name>/` ページが自動生成される SHALL。
...
記事ページに表示される author 名は、対応する `/author/<slug>/` ページへのリンクとして表示される SHALL。
