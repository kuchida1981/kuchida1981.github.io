...
全ページの `<head>` に GA4 の gtag.js スクリプトが挿入される。スクリプトの挿入は本番ビルド（`hugo.IsProduction = true`）のときのみ行われる。
...
GA4 の測定 ID は `config.yml` の `Params.googleAnalyticsID` で管理され、テンプレートはこの値を参照する。
...
`themes/` 配下のファイルを変更せず、`layouts/partials/extend_head.html` のオーバーライドのみで実装される。
