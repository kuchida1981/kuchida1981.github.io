# Capability: Dependabot による CI ゲート付き自動マージ

## Purpose
Dependabot が生成した依存更新PR（GitHub Actions・pip）を対象に、必須ステータスチェック（build・test）が green の場合にのみ自動的にマージすることで、依存更新の追従コストを削減しつつ、CI が red のままの変更や major 更新（破壊的変更の可能性がある更新）が無条件にマージされることを防ぐ。
