...
Posts セクション内の全ページ（記事一覧ページおよび個別記事ページ）の下部に、カテゴリ・タグ・年月アーカイブへのナビゲーションブロックを表示する。ナビゲーションは `layouts/partials/posts_nav.html` として実装し、Posts セクション専用のテンプレートオーバーライドから呼び出す。
...
ナビゲーションブロックに、全カテゴリを件数付きリンクとして表示する。リンク先は Hugo が自動生成するタクソノミーページ (`/categories/<name>/`) とする。
...
ナビゲーションブロックに、記事件数の多い上位15件のタグをリンクとして表示する。全タグ一覧ページ (`/tags/`) へのリンクも併せて表示する。
...
ナビゲーションブロックに、年月別のアーカイブリンクを表示する。リンク先は Posts セクションのセクションページ (`/posts/YYYY/MM/`) とする。
