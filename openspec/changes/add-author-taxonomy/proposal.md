...
記事の author でフィルタ（一覧表示）できる仕組みがない。現在すべての記事に `author` front matter があるが、taxonomy として登録されていないため `/author/<name>/` のようなリストページが生成されない。AI 生成記事（Ghost Writer）と手書き記事（Kosuke Uchida）を区別して閲覧できるようにしたい。
